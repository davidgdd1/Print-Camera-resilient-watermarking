...
TRAIN_PATH = '/media/chuan/新加卷1/coco_dataset/test2017/'
LOGS_Path = "./logs/"
CHECKPOINTS_PATH = './checkpoints/'
SAVED_MODELS = './saved_models'


TRAIN_NOISE_PATH = './train_class'
TRAIN_NOISE_PS_PATH = './train_ps_class'

if not os.path.exists(CHECKPOINTS_PATH):
    os.makedirs(CHECKPOINTS_PATH)

def get_img_batch_nsn(files_list,rand,batch_size=4,size=(400,400)):

    batch_cover = []
    for i in range(batch_size):
        img_cover_path = files_list[rand]
...
def main():
    ...
    parser.add_argument('--exp_name', type=str, default='f100000')
    ...
    parser.add_argument('--JND_loss_scale', type=float, default=1.0)
    parser.add_argument('--JND_loss_ramp', type=int, default=15000)
    ...
    parser.add_argument('--rnd_sat', type=float, default=2.0)
    ...
    parser.add_argument('--rnd_trans_ramp', type=int, default=20000)
    ...
    files_nsn_list = glob.glob(join(TRAIN_NOISE_PATH,"**"))
    print(len(files_nsn_list))
    files_nsn_ps_list = glob.glob(join(TRAIN_NOISE_PS_PATH,"**"))
    ...
    image_nsn_pl = tf.placeholder(shape=[None,height,width,3],dtype=tf.float32,name="input_nsn_hide")
    image_nsn_ps_pl = tf.placeholder(shape=[None, height, width, 3], dtype=tf.float32, name="input_nsn_ps_hide")

    M_pl = tf.placeholder(shape=[None,2,8],dtype=tf.float32,name="input_transform")
    global_step_tensor = tf.Variable(0, trainable=False, name='global_step')
    loss_scales_pl = tf.placeholder(shape=[5],dtype=tf.float32,name="input_loss_scales")
    ...
    nsn = model_f.nsn(height=height, width=width)
    encoder = model_f.StegaStampEncoder(height=height, width=width)
    decoder = model_f.StegaStampDecoder(secret_size=args.secret_size, height=height, width=width)
    discriminator = model_f.Discriminator()

    loss_op, secret_loss_op, yuv_nsn_loss_op, D_loss_op, summary_op, image_summary_op, _ = model_f.build_model(
            encoder=encoder,
            decoder=decoder,
            nsn=nsn,
            ...
            image_nsn_input=image_nsn_pl,
            image_nsn_ps_input=image_nsn_ps_pl,
    ...
    nsn_vars = [var for var in tvars if 'nsn' in var.name]
    ...
    train_nsn_op = tf.train.AdamOptimizer(args.lr).minimize(yuv_nsn_loss_op, var_list=nsn_vars, global_step=global_step_tensor)
    ...
    deploy_hide_image_op, residual_op = model_f.prepare_deployment_hiding_graph(encoder, secret_pl, image_pl)
    deploy_decoder_op =  model_f.prepare_deployment_reveal_graph(decoder, image_pl)
    ...
    # while global_step < args.nsn_num_steps:
    #     for _ in range(min(total_steps, args.num_steps-global_step)):
    #         # rand = np.random.randint(0, len(files_list))
    #         # print(rand)
    #         nsn_images = get_img_batch_nsn(files_list=files_nsn_list,
    #                                                 batch_size=args.batch_size,
    #                                                 size=(height, width))
    #         nsn_images_ps = get_img_batch_nsn(files_list=files_nsn_ps_list,
    #                                                     batch_size=args.batch_size,
    #                                                     size=(height, width))
         
    #         feed_dict = {image_nsn_pl: nsn_images,
    #                      image_nsn_ps_pl: nsn_images_ps,}
    #         _, _, global_step = sess.run([train_nsn_op, yuv_nsn_loss_op, global_step_tensor], feed_dict)
    ...
    while global_step < args.num_steps:
        for _ in range(min(total_steps,args.num_steps-global_step)):
            ...
            images, secrets = get_img_batch(files_list=files_list,
                                                     ...
                                                     batch_size=args.batch_size,
                                                     size=(height,width))
            rand = np.random.randint(0, 7080)
            nsn_images = get_img_batch_nsn(files_list=files_nsn_list,rand=rand,batch_size=args.batch_size,size=(height, width))
            nsn_images_ps = get_img_batch_nsn(files_list=files_nsn_ps_list,rand=rand,batch_size=args.batch_size,size=(height, width))
            ...
            JND_loss_scale = min(args.JND_loss_scale * global_step / args.JND_loss_ramp, args.JND_loss_scale)
            l2_edge_gain = 0
            ...
            feed_dict = {secret_pl:secrets,
                         image_pl:images,
                         image_nsn_pl: nsn_images,
                         image_nsn_ps_pl: nsn_images_ps,
                         M_pl:M,
                         l2_edge_gain_pl:[l2_edge_gain],
                         loss_scales_pl:[l2_loss_scale, lpips_loss_scale, secret_loss_scale, G_loss_scale, JND_loss_scale],
            ...
            if no_im_loss:
                ...
                _, _, _, _, global_step = sess.run([train_op,train_nsn_op,yuv_nsn_loss_op,loss_op,global_step_tensor],feed_dict)
                if not args.no_gan:
            ...
            if global_step % 100 ==0 :
                ...
                summary = tf.Summary(value=[tf.Summary.Value(tag='transformer/rnd_tran', simple_value=rnd_tran),
                                            ...
                                            tf.Summary.Value(tag='loss_scales/JND_loss_scale', simple_value=JND_loss_scale),
